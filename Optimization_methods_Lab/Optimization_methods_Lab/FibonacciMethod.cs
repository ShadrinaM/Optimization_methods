...
namespace Optimization_methods_Lab
{
    public partial class FibonacciMethod : Form
    {
        ...
        {
            // Исходные данные
            double epsilon = 0.2;  //малое число для отступа
            double delta = 0.5;  // Точность
            double a = -3;        // Начало интервала
            double b = 7;         // Конец интервала
            double n_num = Math.Abs((b - a)) / (2 * delta);   // Переменная для подсчёта количества итераций
            ...
            for (k = 0; k < n-1; k++)
            {
                ...
                f_y = f(y);
                f_z = f(z);
                textBox1.AppendText($"Точка y_{k} = {y}, f(y) = {f_y}\r\n");
                textBox1.AppendText($"Точка z_{k} = {z}, f(z) = {f_z}\r\n");
                ...
                if ((b - a) > 2 * delta)
                    textBox1.AppendText($"|L_{NumCulcFun}| = [{a}, {b}] = {b - a} > {2 * delta} => k={k + 1}\r\n");
                else
                    textBox1.AppendText($"|L_{NumCulcFun}| = [{a}, {b}] = {b - a} < {2 * delta}\r\n");
